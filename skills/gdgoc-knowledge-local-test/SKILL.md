...
2. If either tool is unavailable or unauthenticated, stop with
   `temporarily_unavailable`. In a terminal launched for Codex, repair the
   connection with `codex mcp get notion` and `codex mcp login notion`.
3. Invoke this skill as a Codex prompt, for example
